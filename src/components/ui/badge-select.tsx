...
export interface BadgeOption<T extends string = string> {
  ...
  label: string;
  description?: string;
  /** Tailwind classes para o pill (ex: "bg-emerald-500/10 border-emerald-500/30 text-emerald-600 dark:text-emerald-400") */
  bg: string;
  icon: LucideIcon;
}
...
export interface BadgeSelectProps<T extends string = string> {
  ...
  disabled?: boolean;
  /** Quando true, renderiza popover via portal com posicionamento fixed (útil em tabelas com overflow). */
  useFixed?: boolean;
  minWidth?: number;
  placeholder?: string;
  className?: string;
...
export function BadgeSelect<T extends string = string>({
  ...
  // Recalcula posição fixed ao abrir e em scroll/resize
  ...
  // Auto-focus na opção ativa ao navegar pelo teclado
  useEffect(() => {
    if (!open) return;
