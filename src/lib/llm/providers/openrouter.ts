  ...
  async chat(request: ChatRequest): Promise<ChatResult> {
    if (isMockKey(this.apiKey)) {
      ...
      return {
        ...
        usage: {
          tokensInput,
          tokensOutput,
          costUsd: calculateCost(this.model, tokensInput, tokensOutput),
        },
      };
    }
    ...
    const res = await fetch(OPENROUTER_API_URL, {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://nexus-insights.local",
        "X-Title": "Nexus Insights",
      },
      ...
      (tc) => {
        ...
        try {
          parsed = tc.function.arguments
            ? (JSON.parse(tc.function.arguments) as object)
            : {};
        } catch {
