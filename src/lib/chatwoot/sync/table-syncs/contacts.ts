...
/**
 * Sync delta de `contacts` do Chatwoot.
 *
...
async function run({
  ...
  const t0 = Date.now();
  const cursor = await getOrCreateCursor(connectionId, accountId, "contacts");
  const since = cursor.lastSyncedAt ?? EPOCH;

  const sql = `
    SELECT id, account_id, name, email, phone_number, created_at, updated_at
    FROM contacts
    WHERE account_id = $1
      AND updated_at > $2
...
export const contactsSync: TableSync = {
  tableName: "contacts",
