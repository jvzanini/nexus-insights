...
describe("reportingEventsSync", () => {
  it("usa cursor null = ISO 1970 e busca primeiro batch", async () => {
      ...
      rows: [
        {
          ...
          conversation_id: 100,
          user_id: 5,
          event_start_time: new Date("2026-05-04T01:00:00Z"),
          event_end_time: new Date("2026-05-04T01:00:12Z"),
    ...
    const result = await reportingEventsSync.run({
      connectionId: "conn-1",
      accountId: 9,
    });
    ...
    expect(queryNexusChatMock).toHaveBeenCalledWith(
      "conn-1",
      expect.stringContaining("FROM reporting_events"),
      expect.arrayContaining([9, expect.any(Date)]),
    );
    expect(result.tableName).toBe("reporting_events");
