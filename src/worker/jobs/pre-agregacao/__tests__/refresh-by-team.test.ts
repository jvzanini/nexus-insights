/**
 * Testes do job `refresh-by-team` (T6).
 *
 * Mocka pgPool (banco interno) + chatwootQuery (banco Chatwoot) + datetime.
 *
 * Cenários:
 * 1. happy path — 2 accounts × 7 dias × 2 teams (incluindo sentinel 0).
 * 2. idempotente.
 * 3. falha isolada.
...
describe("processRefreshByTeam — happy path", () => {
  it("processa 2 accounts × 7 dias × 2 teams (com sentinel 0)", async () => {
    setupPgPoolHappy();
    setupChatwootHappy();
  ...
  it("usa COALESCE(c.team_id, 0) em todos os SQL agregados", async () => {
    ...
    // Todas as queries com team_id devem usar o sentinel via COALESCE.
    // Snapshot é uma exceção (geramos COALESCE também por consistência).
  ...
  it("UPSERTs incluem team_id = 0 quando vem de orphan team", async () => {
    ...
    const teamZeroUpserts = dailyUpserts.filter((c) => {
      ...
      // params: [account_id, bucket_date, team_id, received, ...]
      return params[2] === 0;
    });
    // 2 accts × 7 dias = 14 UPSERTs com team_id = 0
    ...
    mockedChatwootQuery.mockImplementation(
      async (sql: unknown, params: unknown[] = []) => {
        ...
        if (
          ...
          return [{ team_id: 5, messages_in: 1, messages_out: 1 }] as never;
        }
        if (text.includes("team_id") && text.includes("first_response")) {
          return [{ team_id: 5, frt_p50: 1, frt_p90: 1 }] as never;
        }
        if (text.includes("team_id") && text.includes("conversation_resolved")) {
          return [{ team_id: 5, rt_p50: 1 }] as never;
        }
        if (
          text.includes("team_id") &&
          text.includes("status = 0") &&
          ...
          return [{ team_id: 5, open_at_eod: 0, pending_at_eod: 0 }] as never;
        }
        return [] as never;
      },
    );
