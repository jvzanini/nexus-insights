...
> **Não precisa** de `GHCR_TOKEN` externo. O workflow usa `secrets.GITHUB_TOKEN` (built-in do Actions) com `permissions: packages: read` para puxar a imagem do GHCR — esse token é regenerado a cada run e tem acesso nativo aos packages do mesmo owner. PATs externos (`ghp_...`) são frágeis: expiram, são revogados, ficam fora de sincronia entre projetos.
