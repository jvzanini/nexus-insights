...
> **v3 — pente fino #1 (24 achados) e #2 (30 achados) aplicados. Plan canônico para execução.**
...
## Apêndice B — Pente fino #2 (30 achados aplicados)

| # | Achado | Severidade | Resolução |
|---|---|---|---|
| Q1 | Payload Chatwoot pode ter `account.id` em outro local. | Crítico | Spec §5.2 assume `payload.account.id`. Plan v3 inclui passo: confirmar formato real do Chatwoot via `gh issue` ou docs antes de rodar T4.1. Fallback: também aceitar `payload.message.account_id` se primeiro não existir. |
| Q2 | Endpoint webhook deve estar fora do middleware de auth. | Crítico | T4.1 ganha sub-step: editar `src/middleware.ts` matcher para excluir `/api/webhooks/*`. Validar com test ou curl. |
| Q3 | useFactsRealtime → router.refresh() pode incomodar mobile. | Aceitável | Debounce 5s mitiga. OK. |
| Q4 | instrumentation.ts cleanup hook. | Não-bloqueante | Em produção worker contínuo. Em dev hot reload pode duplicar — guarda em globalThis (mesmo padrão do janitor pool). |
| Q5 | T0.2 audits-table — verificar se realmente existe. | Importante | Subagent que pegar T0.2 deve `grep -rln "Record<AuditAction>" src/ \| grep -i audit` ANTES. Se não houver, T0.2 é skip. |
| Q6 | findUnique com webhookToken NULL/unique. | Confirmado | Postgres aceita múltiplos NULL em UNIQUE. OK. |
| Q7 | createNexusChatConnection após seed Fase 1. | Confirmado | Backfill T3.1 cobre connections existentes sem webhook. |
| Q8 | Race webhook + cron fallback. | Confirmado | Idempotência UPSERT cobre. OK. |
| Q9 | Sticky session no LB. | Confirmado | Stateless. OK. |
| Q10 | req.text() materializa body. | Confirmado | Limite 1MB cobre. OK. |
| Q11 | decrypt() custo CPU. | Confirmado | Negligível em 1.7 req/s. OK. |
| Q12 | Race webhooks no mesmo bucket. | Confirmado | jobId dedup. ESPERADO. |
| Q13 | Webhooks across-bucket podem gerar 2 jobs. | Aceitável | 2s delay min. OK. |
| Q14 | Redis down → endpoint 500. | Crítico | T4.1 ganha try/catch no `redis.incr`: se falha, log warning e continua sem rate limit (degrade gracefully). Test cobre. |
| Q15 | lastWebhookAt fire-and-forget. | Aceitável | Trade-off documentado. OK. |
| Q16 | Sample 1/100 com tráfego baixo invisível. | Aceitável | Log stdout sempre cobre. OK. |
| Q17 | Verificar versão Next.js do projeto. | Crítico | T5.1 ganha pre-step: `cat package.json \| jq '.dependencies.next'`. Se < 15.x, instrumentation requer config. Em Next 16 (atual): automático. |
| Q18 | Schema.prisma como source of truth para TS types. | Confirmado | OK. |
| Q19 | Algoritmo HMAC do Chatwoot. | Importante | Spec assume SHA-256. Verificar no painel admin do Chatwoot na hora do cadastro do webhook (geralmente é configurável). |
| Q20 | URL de 64 chars dentro de URL limits. | Confirmado | OK. |
| Q21 | Webhook não precisa CORS. | Confirmado | OK. |
| Q22 | Mock decrypt no teste. | Importante | T4.1 detalha: `jest.mock("@/lib/encryption", () => ({ decrypt: jest.fn().mockReturnValue(KNOWN_SECRET) }))` + computar HMAC do test body usando KNOWN_SECRET. |
| Q23 | testEnvironment: 'node' globalmente. | Confirmado | jest.config.ts já tem `testEnvironment: "node"`. OK. |
| Q24 | T0.2 audits-table — TODO grep first. | Importante | Documentado em Q5. Subagent decide. |
| Q25 | URL 64 chars OK. | Confirmado | OK. |
| Q26 | Após push, cadastrar webhooks no Chatwoot manualmente. | Importante | Runbook §13.1 cobre. T9.0 cria runbook. |
| Q27 | Traefik routes /api/* corretamente. | Crítico | T9.1 inclui smoke test: `curl -I https://insights.nexusai360.com/api/webhooks/nexus-chat/teste-token-invalido` deve retornar 404 (não 502 ou Bad Gateway). Se Bad Gateway, Traefik label não cobre. |
| Q28 | 100 req/min/token arbitrário. | Confirmado | Volume real Chatwoot ~6k/h = 100/min teórico. Margem ok. |
| Q29 | Cron fallback processa todos bindings. | Confirmado | Design correto. |
| Q30 | Validar cron 30 min em produção. | Importante | T9.1 inclui validação: após deploy, monitorar logs por 35 min e confirmar que cron fallback dispara 1x. Audit log busca `worker.refresh-by-account` events. |
